...
from nets.convnetpspnet import get_convnet_encoder
...
def pool_block(feats, pool_factor, out_channel):
	...
	x = Activation('relu' )(x)
	#x = Lambda(lambda x: K.resize_images(x, (K.int_shape(feats)[1], K.int_shape(feats)[2]), data_format="channels_last",interpolation='bilinear'))(x)
	x = UpSampling2D((K.int_shape(feats)[1]//K.int_shape(x)[1],K.int_shape(feats)[2]//K.int_shape(x)[2]),interpolation='bilinear')(x)
	return x

def pspnet(n_classes, input_height, input_width,downsample_factor=8, backbone='mobilenet', aux_branch=True):
	if backbone == "mobilenet":
		img_input, f4, o = get_mobilenet_encoder(inputs_size, downsample_factor=downsample_factor)
		out_channel = 320
	elif backbone == "vgg16":
		img_input, f4, o = get_convnet_encoder(input_height,input_width)
		out_channel = 512
	elif backbone == "resnet50":
		img_input, f4, o = get_resnet50_encoder(input_height,input_width)
		out_channel = 2048
	else:
...
#	o = Lambda(lambda x: tf.image.resize_images(x, (inputs_size[1], inputs_size[0]), align_corners=True))(o)
	# 获得每一个像素点属于每一个类的概率了
	o = UpSampling2D((K.int_shape(img_input)[1]//K.int_shape(o)[1],K.int_shape(img_input)[2]//K.int_shape(o)[2]),interpolation='bilinear')(o)
...
#		f4 = Lambda(lambda x: tf.image.resize_images(x, (inputs_size[1], inputs_size[0]), align_corners=True))(f4)
		f4 = UpSampling2D((K.int_shape(img_input)[1]//K.int_shape(f4)[1],K.int_shape(img_input)[2]//K.int_shape(f4)[2]),interpolation='bilinear')(f4)
		# 获得每一个像素点属于每一个类的概率了
		f4 = Activation("sigmoid", name="aux")(f4)
