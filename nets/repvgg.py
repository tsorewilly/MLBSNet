...
class conv_fuse_bn(tf.keras.layers.Layer):
    def __init__(
            self,
            in_channels,
            out_channels,
        ...
        super(conv_fuse_bn, self).__init__()
        self.deploy = deploy
        self.groups = groups
        ...
        if deploy:
            self.rbr_reparam = tf.keras.Sequential( #用于前向传播的层
                [
                    tf.keras.layers.ZeroPadding2D(padding=padding),
                    tf.keras.layers.Conv2D(
                        filters=out_channels,
            ...
            self.rbr_dense = conv_bn(  #Squential 用来真正训练的层
    ...
    def _fuse_bn_tensor(self, branch):
        ...
        t = gamma / std
        return kernel * t, beta - running_mean * gamma / std #即代入简单网络的偏差层来自bn

    def repvgg_convert(self): #训练层参数转换为前向传播层参数
...
#                        groups=groups,
            ...
            self.rbr_identity = (  #非Squential
                tf.keras.layers.BatchNormalization()
                if out_channels == in_channels and strides == 1
                else None
            )
            self.rbr_dense = conv_bn(  #Squential
                out_channels=out_channels,
                kernel_size=kernel_size,
                strides=strides,
                padding=padding,
#                groups=groups,
            ...
            self.rbr_1x1 = conv_bn(  #Squential
                out_channels=out_channels,
                kernel_size=1,
                strides=strides,
                padding=padding_11,
#                groups=groups,
    ...
    def get_equivalent_kernel_bias(self):
        ...
        kernelid, biasid = self._fuse_bn_tensor(self.rbr_identity) #正向传播时，返回identity层的kernel（逐个逐层中间为1）和训练出来的bn中的参数融合
        return (
            kernel3x3 + self._pad_1x1_to_3x3_tensor(kernel1x1) + kernelid, #对于1*1,先bn再pad等价先pad再bn
    ...
    def _fuse_bn_tensor(self, branch):
        ...
        if isinstance(branch, tf.keras.Sequential):
            ...
            if not hasattr(self, "id_tensor"): #identity卷积核只需生成一次
...
class RepVGG(tf.keras.Model):
    def __init__(
        ...
        self.stage0 = RepVGGBlock(   #非Sequential
            in_channels=3,
        ...
        self.stage1 = self._make_stage( #Sequential
...
def repvgg_model_convert(
    model: tf.keras.Model, build_func, NCLASSES,save_path=None, image_size=(416, 416, 1)
):
    deploy_model = build_func(n_classes=NCLASSES,input_height=416, input_width=416,deploy=True)
    ...
    for layer, deploy_layer in zip(model.layers, deploy_model.layers):
        if hasattr(layer, "repvgg_convert"): #只能是RepVGGBlock
            kernel, bias = layer.repvgg_convert()
            deploy_layer.rbr_reparam.layers[1].set_weights([kernel, bias])
        # elif isinstance(layer, tf.keras.Sequential): #只能是_make_stage
        ...
        #     deploy_layer.set_weights(weights)
            ...
            assert isinstance(deploy_layer, tf.keras.layers.Conv2D)
            weights = layer.get_weights()
            deploy_layer.set_weights(weights)  #只与卷积核的形状有关，和输入大小无关
        elif isinstance(layer, tf.keras.layers.Conv2DTranspose):
            assert isinstance(deploy_layer, tf.keras.layers.Conv2DTranspose)
            weights = layer.get_weights()
            deploy_layer.set_weights(weights)
        elif isinstance(layer, tf.keras.layers.BatchNormalization):
            assert isinstance(deploy_layer, tf.keras.layers.BatchNormalization)
            weights = layer.get_weights()
            deploy_layer.set_weights(weights)
        elif isinstance(layer, tf.keras.layers.DepthwiseConv2D):
            assert isinstance(deploy_layer, tf.keras.layers.DepthwiseConv2D)
            weights = layer.get_weights()
            deploy_layer.set_weights(weights)
        elif isinstance(layer, tf.keras.layers.Dense):
            assert isinstance(deploy_layer, tf.keras.layers.Dense)
...
#     ):
#     deploy_model = build_func(n_classes=2,input_height=384, input_width=384,deploy=True)
#     deploy_model.build(input_shape=(None, *image_size))